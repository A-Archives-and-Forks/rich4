...
mov word [(_all_special_players_state + 64)], dx  ; mov word [0x498e68], dx
mov dx, word [eax + (_all_players_state + 10)]  ; mov dx, word [eax + 0x496b72]
mov word [(_all_special_players_state + 66)], dx  ; mov word [0x498e6a], dx
mov dx, word [eax + (_all_players_state + 12)]  ; mov dx, word [eax + 0x496b74]
mov word [(_all_special_players_state + 68)], dx  ; mov word [0x498e6c], dx
mov dx, word [eax + (_all_players_state + 14)]  ; mov dx, word [eax + 0x496b76]
mov word [(_all_special_players_state + 70)], dx  ; mov word [0x498e6e], dx
mov dl, byte [_current_player]  ; mov dl, byte [0x49910c]
mov byte [(_all_special_players_state + 72)], dl  ; mov byte [0x498e70], dl
mov al, byte [eax + (_all_players_state + 16)]  ; mov al, byte [eax + 0x496b78]
mov byte [(_all_special_players_state + 73)], al  ; mov byte [0x498e71], al
xor ah, ah
mov byte [(_all_special_players_state + 74)], ah  ; mov byte [0x498e72], ah
